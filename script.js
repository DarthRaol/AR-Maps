...
.setLngLat([72.83441586635318, 19.060396215930364]) // main
.addTo(map);

locationDiv[0].addEventListener('click', () => {
    directions.setDestination([72.83441586635318, 19.060396215930364]);
    DisableMarkers(0);
});

const mountMaryMarker = new mapboxgl.Marker(locationDiv[1])
.setLngLat([72.82217957369458, 19.04654194640622]) // Marker in Mount Mary
.addTo(map);

locationDiv[1].addEventListener('click', () => {
    DisableMarkers(1);
    directions.setDestination([72.82217957369458, 19.04654194640622]);
});
